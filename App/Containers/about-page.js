...
import { SectionWrapper, Row, Column } from '../Components/styleLayout';
...
export default function AboutMe() {
  ...
  const scroll = (name, width) => {
    let el = [aboutEl, workEl, projectEl].filter(
      ref => ref.current.id === name
    )[0];

    const { offsetTop } = el.current.container;
    ...
    function animate(timestamp) {
      ...
      let changePercent =
        progress >= duration ? 1 : defaultEasing(progress / duration);
  ...
  return (
    <ThemeProvider theme={theme}>
      <AppWrapper>
        <GlobalStyle />
        <Navigation cb={changeTheme} scroll={scroll} />
        <SocialLinks ref={linksEl} />
        <Column>
          <React.Suspense fallback={''}>
            <Intro ref={introEl} />
            <About ref={aboutEl} />
            <Work ref={workEl} />
            <Projects ref={projectEl} />
            <Farewell />
            <Footer />
          </React.Suspense>
        </Column>
